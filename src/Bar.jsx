import React, { useEffect } from "react";
import { useHistory, useLocation } from "react-router-dom";

import { Ø, ØØ, debounce } from "./lib";
import { render } from "./mobile";
import { openMenu } from "./Menu";

export default (props) => {
  const history = useHistory();
  const search = async (keyword) => {
    if (keyword.trim()) {
      history.replace(`/search/${encodeURIComponent(keyword)}/`);
    } else {
      history.replace("/search/");
    }
    await render();
  };
  const debouncedSearch = debounce(search, 500);

  let location = useLocation();
  useEffect(() => {
    const [season, title] = location.pathname
      .split("/")
      .filter((e) => e)
      .map((e) => decodeURIComponent(e));
    if (season === "search") {
      Ø(".search").value = title || "";
      if (!Ø(".search").value) {
        Ø(".search").focus();
      }
    }
  }, [location]);

  const [season, title] = location.pathname
    .split("/")
    .filter((e) => e)
    .map((e) => decodeURIComponent(e));

  return (
    <div className="bar">
      <div className="icon" onClick={async () => await openMenu()}>
        <img src="data:image/webp;base64," />
      </div>
      <div className={season === "search" ? "title hidden" : "title"}>
        {title || season || "カリ(仮)"}
      </div>
      <input
        className={season === "search" ? "search" : "search hidden"}
        type="text"
        placeholder="搜尋"
        autoComplete="off"
        onFocus={async (e) => await search(e.target.value)}
        onInput={async (e) => await debouncedSearch(e.target.value)}
      />
      <button
        onClick={() => {
          ØØ(".list .item").forEach((e) => (e.onclick = null));
          const keyword = Ø(".search").value;
          if (location.pathname.split("/").filter((e) => e)[0] === "search") {
            if (keyword.trim()) {
              history.push("/search/");
            } else {
              history.push("/");
            }
          } else {
            if (keyword.trim()) {
              history.push(`/search/${encodeURIComponent(keyword)}/`);
            } else {
              history.push("/search/");
            }
          }
          render();
          return;
        }}
      >
        {season === "search" ? (title ? "清除" : "取消") : "搜尋"}
      </button>
    </div>
  );
};
